...
1. **加电荷求电压法**：设导体上带有电荷 $q$，用高斯定理求 $\vec{E}$，用线积分求 $U$，得 $C = \frac{q}{U}$。
2. **加电压求电荷法**：设导体间电压为 $U$，用 Laplace 方程解出 $\varphi$，取梯度求 $\vec{E}$，由导体表面的 $\vec{D} = \rho_s \vec{n}$ 和积分求 $q$，从而求得 $C = \frac{q}{U}$。
3. **电场能量法**：设 $U$（可设为 1），求出 $\varphi$ 和 $\vec{E}$，由整个空间的 $\vec{E}$ 的体积分求电场能量 $W_e$。根据 $W_e = \frac{1}{2} C U^2$ 求电容 $C$；或加电荷求能量 $W_e = \frac{1}{2} \frac{q^2}{C}$ 求 $C$。
...
在比拟物理量时，静电场中导体表面 $\vec{E}$ 总是垂直于表面（相当于介介常数 $\varepsilon \to \infty$）；在电流场中，与之比拟的是高导电率的电极（$\sigma \to \infty$），其电流线也垂直于表面。
...
**计算方法：**
1. **设电流求电压法**：设电流 $I$，求得 $\vec{J}$，由 $\vec{E} = \frac{\vec{J}}{\sigma}$ 求电场，积分求得 $U = \int \vec{E} \cdot d\vec{l}$。
2. **设电压求电流法**：设电位差 $U$，求出 $\varphi$，得到 $\vec{E} = -\nabla\varphi$，求得 $\vec{J} = \sigma \vec{E}$，最后积分得到 $I = \iint_S \vec{J} \cdot d\vec{S}$。
3. **功耗法**：
...
**例题**：求图示同心半球壳电极间的电导，球壳半径为 $a$ 和 $b$，介质电导率为 $\sigma$。
...
控制方程：
$$ \nabla^2 \varphi = 0 $$
在球坐标系下，由于对称性电位只与径向 $r$ 相关：
$$ \frac{d}{dr} \left( r^2 \frac{d\varphi}{dr} \right) = 0 $$
边界条件为：
$$ \varphi(a) = 0, \quad \varphi(b) = U $$
通解为：
$$ \varphi(r) = -\frac{C_1}{r} + C_2 $$
代入边界条件解得：
$$ C_1 = \frac{U a b}{b - a}, \quad C_2 = \frac{U b}{b - a} $$
所以电电分布为：
$$ \varphi(r) = \frac{U a b}{b - a} \left( \frac{1}{a} - \frac{1}{r} \right) $$
电场强度为：
$$ \vec{E} = -\nabla\varphi = -\frac{\partial\varphi}{\partial r} \vec{e}_r = -\frac{U a b}{b - a} \frac{1}{r^2} \vec{e}_r $$
电流密度为：
$$ \vec{J} = \sigma \vec{E} = \frac{\sigma U a b}{b - a} \frac{1}{r^2} \vec{e}_r $$
单位高度或半球面上的总电流为（积分面为 $S = 2\pi r^2$）：
$$ I = \iint_S \vec{J} \cdot d\vec{S} = J \cdot 2\pi r^2 = \frac{2\pi\sigma U a b}{b - a} $$
故求得电导为：
$$ G = \frac{I}{U} = \frac{2\pi\sigma a b}{b - a} $$
...
安培环路定理表明：磁感应强度 $\vec{B}$ 沿任意闭合路径的线积分，等于该闭合路径所包围并交链的穿过表面的总电流与真空磁导率的乘积。环路方向与穿过的电流方向成右手螺旋关系时电流取正号，反之取负号。
